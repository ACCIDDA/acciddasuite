...
#' \describe{
#'   \item{\code{accidda_data}}{Observed counts, one row per series and
#'     \code{target_end_date} (latest reported value per date when revisions
#'     are present).}
#'   \item{\code{accidda_ncast}}{Weekly nowcast summary, one row per series and
...
#'   \item{\code{accidda_fcast}}{Forecast quantiles per model, one row per
...
#' }
#'
#' @param x An \code{accidda_data}, \code{accidda_ncast} or
#'   \code{accidda_fcast} object.
#' @param ... Ignored.
#' @return A tibble.
#' @seealso \code{\link{autoplot.accidda_data}},
#'   \code{\link{autoplot.accidda_ncast}}, \code{\link{autoplot.accidda_fcast}}
#' @examples
#' example_data |>
#'   check_data() |>
#'   as_tibble()
#' @export
as_tibble.accidda_data <- function(x, ...) {
  dplyr::as_tibble(extract_series(x))
}


#' @rdname as_tibble.accidda_data
#' @export
as_tibble.accidda_ncast <- function(x, ...) {
...
#' @rdname as_tibble.accidda_data
#' @export
as_tibble.accidda_fcast <- function(x, ...) {
  x$hub$model_out_tbl |>
